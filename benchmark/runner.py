...
class BenchmarkRunner:
    ...
    def run(self) -> None:
        ...
        experiments_per_lang = self._build_experiment_block()
        csv_path = self.results_dir / f"benchmark_{datetime.now():%Y%m%d_%H%M%S}.csv"

        print(f"Experimento: {len(self.config['languages'])} lenguajes x "
              f"{len(self.config['algorithms'])} algoritmos x "
              f"2 escenarios x {self.config['experiment']['replicas']} replicas")
        print(f"Total: {sum(len(e) for e in experiments_per_lang)} ejecuciones de k6")
        print(f"Semilla: {self.config['experiment']['seed']}")
        print(f"CSV: {csv_path}")
        print()

        if self.dry_run:
            self._print_dry_run(experiments_per_lang)
            return
        ...
        with open(csv_path, "w", newline="", encoding="utf-8") as csvfile:
            ...
            for i, lang in enumerate(self.config["languages"]):
                ...
                exps = experiments_per_lang[i]
                self._run_language_block(lang, exps, writer, csvfile)
    ...
    # -------------------------------------------------------------------
    # Bloque por lenguaje (build -> start -> experiments -> stop)
    # -------------------------------------------------------------------

    def _run_language_block(
        self,
        lang: Dict[str, Any],
        ...
        name = lang["name"]
        print(f"{'='*60}")
        print(f"  LENGUAJE: {name.upper()}  ({len(experiments)} ejecuciones)")
        print(f"{'='*60}")
        ...
        try:
            self._wait_for_health(name, lang["port"])

            # Shuffle experiments within this language block
            ...
            for idx, exp in enumerate(experiments):
                ...
                label = f"{name} | {exp['algorithm']} | {exp['scenario']} | rep {exp['replica']}"
                print(f"  {progress} {label}  ", end="", flush=True)

                try:
                    result = self._run_single_k6(name, lang["port"], exp)
    ...
    def _run_single_k6(
        ...
        dur = metrics["request_duration"]["values"]
        err = metrics["error_rate"]["values"]
        reqs = metrics["total_requests"]["values"]
        ...
        return {
            ...
            "sample_size": int(dur.get("count", 0)),
            "error_rate": round(err.get("rate", 0), 6),
    ...
    def _build_experiment_block(self) -> List[List[Dict[str, Any]]]:
        """
        Retorna una lista de bloques (uno por lenguaje), donde cada bloque
        contiene la lista de experimentos para ese lenguaje.
        """
    ...
    def _print_dry_run(self, blocks: List[List[Dict[str, Any]]]) -> None:
        ...
        for i, lang in enumerate(self.config["languages"]):
            ...
            print(f"  {name.upper()}  ({len(shuffled)} ejecuciones)")
            for exp in shuffled[:5]:
                print(f"    {exp['algorithm']:10s} | {exp['scenario']:4s} | rep {exp['replica']}")
            if len(shuffled) > 5:
                print(f"    ... y {len(shuffled) - 5} mas")
            print()
